...
const getFalseRentasUsuario =  (req, res) => {
  jwt.verify(req.token,'seguridadAmbar', (error, authData) =>{
    try {
      const falseEstadoRenta =  RentaUsuario.find({ estadoRenta: false });
...
const getFinalizacion =  (req, res) => {
  jwt.verify(req.token,'seguridadAmbar', (error, authData) =>{
    try {
      const finalizacion =  RentaUsuario.find({
        estadoRenta: true,
...
const getIdRenta = (req, res)=>{
  jwt.verify(req.token,'seguridadAmbar', (error, authData) =>{
    try{
      console.log("HOLA");
      let returnRenta=[];
      const rentasUsuario =  RentaUsuario.findOne({_id: req.params.idRenta}).exec()
      if (!rentasUsuario) {
...
const createRentaUsuario =  (req, res) => {
  jwt.verify(req.token,'seguridadAmbar', (error, authData) =>{
    const nuevaRenta = {
...
///
const updateSeEjecutoConExitoLarenta =  (req, res) => {
  jwt.verify(req.token,'seguridadAmbar', (error, authData) =>{
